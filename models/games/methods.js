...
const SelectOne = async (data, sort = {},limit=20) => {
    const db = await mongo.get();
    return await db.collection(tablename).find(data).sort(sort).limit(limit).toArray()
};
...
const Insert = async (data,session) =>{
    const db = await mongo.get();
    return await db.collection(tablename).insertOne(data,{ session })
}
